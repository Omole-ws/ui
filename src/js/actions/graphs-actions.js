...
export function importGraph(exp) {
    return function(dispatch) {
        dispatch({ type: `${ActionType.GRAPH_IMPORT}_PENDING`, payload: exp })
        netAction({
            url: `${DataURL}/import`,
            method: 'post',
            body: exp,
            onSuccess: payload => dispatch({ type: `${ActionType.GRAPH_IMPORT}_OK`, payload }),
            onError: error => dispatch({ type: `${ActionType.GRAPH_IMPORT}_FAIL`, payload: exp, error })
        })
    }
}
